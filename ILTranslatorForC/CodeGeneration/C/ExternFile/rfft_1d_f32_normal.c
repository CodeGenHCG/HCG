...
void rfft_1d_f32_normal(int n, float* in, float* out)
{
    float* xIm = (float*)malloc(n*sizeof(float));
    float* yIm = (float*)malloc(n*sizeof(float));
    memset(xIm, 0, n*sizeof(float));
    fft(n, in, xIm, out, yIm);
    free(xIm);
    free(yIm);
}
